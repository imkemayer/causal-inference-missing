...
load("pcoef_scenario3.Rdata")
load("ycoef_scenario3.Rdata")

pcoef<-pcoef_scenario3
ycoef<-ycoef_scenario3
...
Vselect1<-c(1,2,3,4,5,11,12,13,14,19,20,21,25,26,29)
Vselect2<-Vselect1+30
Vselect3<-Vselect2+30
Vselect4<-Vselect3+30
Vselect5<-Vselect4+30
Vselect6<-Vselect5+30

Vselect<-c(Vselect1,Vselect2,Vselect3,Vselect4,Vselect5,Vselect6)

Vtreatment<-Va[,Vselect]

Vint<-cbind(1,Vtreatment)

probexpint<-apply(t(t(Vint)*pcoef),1,sum)
...
# Create CSV files
csvFileName <- paste("CHDScenario3DS",k,".csv",sep="")
...
DS_scenario3<-DS
save(DS_scenario3,file="DS_scenario3.Rdata")

ate_scenario3<-ate
save(ate_scenario3,file="ate_scenario3.Rdata")
