...
load("pcoef_scenario1.Rdata")
load("ycoef_scenario1.Rdata")

pcoef<-pcoef_scenario1
ycoef<-ycoef_scenario1
...
meany<- apply(t(t(V)*ycoef),1,sum)+0.2*A
err<-rnorm(n,sd=2) 

Y<-meany+err 

meany11<- apply(t(t(V)*ycoef),1,sum)+0.2*1
meany10<- apply(t(t(V)*ycoef),1,sum)+0.2*0
...
# Create CSV files
csvFileName <- paste("CHDScenario1DS",k,".csv",sep="")
...
DS_scenario1<-DS
save(DS_scenario1,file="DS_scenario1.Rdata")

ate_scenario1<-ate
save(ate_scenario1,file="ate_scenario1.Rdata")
