...
load("pcoef_scenario8.Rdata")
load("ycoef_scenario8.Rdata")

pcoef<-pcoef_scenario8
ycoef<-ycoef_scenario8
...
ooverV16<-1/(V[,16])
pabsV51<-abs(V[,51])
intlogp1V7V156<-V[,156]/(logp1V7+0.5)
squareV163<-(V[,163]^2)

Vtreatment<-cbind(1,logp1V7,ooverV16,pabsV51,V[,156],intlogp1V7V156,squareV163)
...
VYa<-cbind(V[,7],V[,16],V[,51],V[156])
VY<-data.frame(1,VYa)

meany1a<-apply(t(t(VY)*ycoef),1,sum)-10*A
meany1<-10/(2+exp(-1*meany1a))

err<-rnorm(n,sd=0.2) 

Y<-meany1+err

meany11a<-apply(t(t(VY)*ycoef),1,sum)-10*1
meany11<-10/(2+exp(-1*meany11a))

meany10a<-apply(t(t(VY)*ycoef),1,sum)-10*0
meany10<-10/(2+exp(-1*meany10a))
...
csvFileName <- paste("CHDScenario8DS",k,".csv",sep="")
...
DS_scenario8<-DS
save(DS_scenario8,file="DS_scenario8.Rdata")
ate_scenario8<-ate
save(ate_scenario8,file="ate_scenario8.Rdata")

#> summary(ate)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-1.474  -1.440  -1.430  -1.432  -1.423  -1.401 
