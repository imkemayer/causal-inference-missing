...
load("pcoef_scenario6.Rdata")
load("ycoef_scenario6.Rdata")

pcoef<-pcoef_scenario6
ycoef<-ycoef_scenario6
...
Vtreatment<-cbind(1,V[,4],V[,19],V[,44])
...
VYa<-cbind(V[,4],V[,19],V[,44])

VY<-data.frame(1,VYa)

meany1<- apply(t(t(VY)*ycoef),1,sum)-0.4*A-0.2*(A*V[,55])

err<-rt(n,df=19)
logY<-meany1+err 

meany11<- apply(t(t(VY)*ycoef),1,sum)-0.4*1-0.2*V[,55]*1
meany10<- apply(t(t(VY)*ycoef),1,sum)-0.4*0-0.2*V[,55]*0
...
# Create CSV files
csvFileName <- paste("CHDScenario6DS",k,".csv",sep="")
...
DS_scenario6<-DS
save(DS_scenario6,file="DS_scenario6.Rdata")

ate_scenario6<-ate
save(ate_scenario6,file="ate_scenario6.Rdata")

#> summary(ate)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-1.3248 -1.1329 -1.1021 -1.1039 -1.0519 -0.9104 
