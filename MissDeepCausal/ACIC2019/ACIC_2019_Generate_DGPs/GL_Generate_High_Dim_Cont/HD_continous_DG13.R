...
load("pcoef_scenario13.Rdata")
load("ycoef_scenario13.Rdata")

pcoef<-pcoef_scenario13
ycoef<-ycoef_scenario13
...
intcontV5<-(V[,5]-200)*(V[,5]>204)
sqrtV25<-sqrt(V[,25])
intintcontV5V5sqrtV25<-intcontV5*sqrtV25
intV96V163<-V[,96]*V[,163]
squareV163<-(V[,163]^2)
pabsV169<-abs(V[,169])
cubeV188<-V[,188]^3
intpabsV169cubeV188<-pabsV169*cubeV188

Vtreatment<-cbind(1,intcontV5,sqrtV25,intintcontV5V5sqrtV25,V[,96],V[,163],intV96V163,squareV163,pabsV169,cubeV188,intpabsV169cubeV188)
...
VYa<-cbind(intcontV5,logp1V7,ooverV16,pabsV51,V[,96],intlogp1V7V96,V[,156],squareV163,squareV188)

VY<-data.frame(1,VYa)

meany1<-apply(t(t(VY)*ycoef),1,sum)-0.5*A-0.25*A*squareV163
...
meany11<-apply(t(t(VY)*ycoef),1,sum)-0.5*1-0.25*1*squareV163
meany10<-apply(t(t(VY)*ycoef),1,sum)-0.5*0-0.25*0*squareV163
...
csvFileName <- paste("CHDScenario13DS",k,".csv",sep="")
...
DS_scenario13<-DS
save(DS_scenario13,file="DS_scenario13.Rdata")
ate_scenario13<-ate
save(ate_scenario13,file="ate_scenario13.Rdata")

#> summary(ate)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-0.9652 -0.8728 -0.8493 -0.8486 -0.8267 -0.7753 