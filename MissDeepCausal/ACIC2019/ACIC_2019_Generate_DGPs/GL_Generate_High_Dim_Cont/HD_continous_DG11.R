...
load("pcoef_scenario11.Rdata")
load("ycoef_scenario11.Rdata")

pcoef<-pcoef_scenario11
ycoef<-ycoef_scenario11
...
Vselect<-c(6,7,8,9,10,15,16,17,18,23,24,28,29,30,36,37,38,39)

Vtreatment<-cbind(1,Va[,Vselect])
...
VY<-Va[,c(1:5,Vselect)]
meany1<- apply(t(t(VY)*ycoef),1,sum)+1*A

err<-rnorm(n,0,sd=2)

Y<-(meany1)^2+err

meany11<- apply(t(t(VY)*ycoef),1,sum)+1*1
meany10<- apply(t(t(VY)*ycoef),1,sum)+1*0
...
csvFileName <- paste("CHDScenario11DS",k,".csv",sep="")
...
DS_scenario11<-DS
save(DS_scenario11,file="DS_scenario11.Rdata")
ate_scenario11<-ate
save(ate_scenario11,file="ate_scenario11.Rdata")

#> summary(ate)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#10.58   10.74   10.78   10.77   10.81   10.92 
