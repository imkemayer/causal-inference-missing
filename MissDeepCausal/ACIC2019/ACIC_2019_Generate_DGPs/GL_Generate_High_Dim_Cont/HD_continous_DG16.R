...
# Final reshuffle of covariates
set.seed(78457)
IndF<-sample(3:(ncov+2),ncov)

# load coefficient regressions for treatment and outcome models (resp)
load("pcoef_scenario16.Rdata")
load("ycoef_scenario16.Rdata")

pcoef<-pcoef_scenario16
ycoef<-ycoef_scenario16
...
meany<- apply(t(t(V)*ycoef),1,sum)+1*A

err<-rt(n,df=8) 

Y<-meany+err 

meany11<- apply(t(t(V)*ycoef),1,sum)+1*1
meany10<- apply(t(t(V)*ycoef),1,sum)+1*0
...
# Final reshuffle of data

CS<-data.frame(datatest[,c(1:2,IndF)])

for (i in 3:ncol(CS)) {
   colnames(CS)[i] <- paste("V", i-2, sep="")
}

csvFileName <- paste("CHDScenario16DS",k,".csv",sep="")
...
DS_scenario16<-DS
save(DS_scenario16,file="DS_scenario16.Rdata")

ate_scenario16<-ate
save(ate_scenario16,file="ate_scenario16.Rdata")
