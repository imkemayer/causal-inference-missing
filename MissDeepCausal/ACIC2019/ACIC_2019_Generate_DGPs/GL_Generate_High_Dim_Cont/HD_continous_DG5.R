...
load("pcoef_scenario5.Rdata")
load("ycoef_scenario5.Rdata")

pcoef<-pcoef_scenario5
ycoef<-ycoef_scenario5
...
sdV5<-4*sd(V[,5])
intV2V5<-V[,2]*V[,5]
sdintV2V5<-4*sd(intV2V5)

sdV23<-4*sd(V[,23])

V23square<-V[,23]^2
sdV23square<-4*sd(V23square)

intV12V23square<-V[,12]*V23square
sdintV12V23square<-4*sd(intV12V23square) 

sqrtV67<-sqrt(V[,67])
sdsqrtV67<-4*sd(sqrtV67)

IndV89<-V[,89]>19

IndcontV95<-(V[,95]-3)*(V[,95]>5)
sdIndcontV95<-4*sd(IndcontV95)

expV106<-exp(V[,106])
sdexpV106<-sd(expV106)

sdV122<-4*sd(V[,122])
sdV146<-4*sd(V[,146])

intV122V146<-V[,122]*V[,146]
sdintV122V146<-4*sd(intV122V146)

sdV168<-4*sd(V[,168])
IndV199<-V[,199]>1

Vtreatment<-cbind(1,V[,23],V23square,sqrtV67,V[,77],IndV89,
                  IndcontV95,expV106,V[,122],V[,146],intV122V146,V[,150],V[,168],IndV199)
...
VYa<-cbind(sqrtV67,V[,77],IndV89,IndcontV95,expV106,V[,122],V[,146],intV122V146,V[,150],V[,168],IndV199)

VY<-data.frame(1,VYa)

meany1<- apply(t(t(VY)*ycoef),1,sum)-0.4*A

err<-rt(n,df=12)
logY<-meany1+err 

meany11<- apply(t(t(VY)*ycoef),1,sum)-0.4*1
meany10<- apply(t(t(VY)*ycoef),1,sum)-0.4*0
...
# Create CSV files
csvFileName <- paste("CHDScenario5DS",k,".csv",sep="")
...
save(DS_scenario5,file="DS_scenario5.Rdata")
save(ate_scenario5,file="ate_scenario5.Rdata")

# > summary(ate)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -1.0804 -0.3602 -0.3321 -0.3507 -0.3173 -0.2850 
