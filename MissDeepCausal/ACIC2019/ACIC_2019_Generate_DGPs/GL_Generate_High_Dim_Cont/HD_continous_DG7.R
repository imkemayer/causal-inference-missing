...
load("pcoef_scenario7.Rdata")
load("ycoef_scenario7.Rdata")

pcoef<-pcoef_scenario7
ycoef<-ycoef_scenario7
...
V24square<-V[,24]^2
sqrtV68<-sqrt(V[,68])
intV35V68sqrt<-V[,35]*sqrtV68
IndcontV96<-(V[,96]-1)*(V[,96]<2)
expV107<-exp(V[,107])
intV123V149<-V[,123]*V[,149]
ooverV169<-1/(V[,169])

Vtreatment<-cbind(1,V[,3],V[,6],intV3V6,V[,24],V24square,V[,35],V[,68],sqrtV68,intV35V68sqrt,IndcontV96,expV107,
                  V[,123],V[,149],intV123V149,V[,151],ooverV169,V[,200])
...
VYa<-cbind(V[,3],V[,6],intV3V6,V[,24],V24square,V[,35],V[,40],V[,68],sqrtV68,intV35V68sqrt,
           IndcontV96,expV107,V[,123],V[,133],V[,149],intV123V149,V[,151],ooverV169,V[,198],V[,200])

VY<-data.frame(1,VYa)

meany1<- apply(t(t(VY)*ycoef),1,sum)+0*A

err<-rnorm(n) 
Y<-meany1+err 

meany11<- apply(t(t(VY)*ycoef),1,sum)+0*1
meany10<- apply(t(t(VY)*ycoef),1,sum)+0*0
...
csvFileName <- paste("CHDScenario7DS",k,".csv",sep="")
...
DS_scenario7<-DS
save(DS_scenario7,file="DS_scenario7.Rdata")

ate_scenario7<-ate
save(ate_scenario7,file="ate_scenario7.Rdata")
