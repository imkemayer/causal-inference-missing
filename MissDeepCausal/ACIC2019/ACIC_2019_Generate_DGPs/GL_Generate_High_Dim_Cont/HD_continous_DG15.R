...
load("pcoef_scenario15.Rdata")
load("ycoef_scenario15.Rdata")

pcoef<-pcoef_scenario15
ycoef<-ycoef_scenario15
...
intcontV10<-(V[,10]-5)*(V[,10]>5)
sqrtV25<-sqrt(V[,25])
intintcontV10sqrtV25<-intcontV10*sqrtV25
logV52<-log(V[,52])
root3absV70<-abs(V[,70])^{1/3}
intlogV52V96<-logV52*V[,96]
intV96V158<-V[,96]*V[,158]
squareV163<-(V[,163]^2)
pabsV169<-abs(V[,169])
cubeV188<-V[,188]^3
intcubeV188V192<-cubeV188*V[,192]

Vtreatment<-cbind(1,intcontV10,sqrtV25,intintcontV10sqrtV25,V[,42],logV52,root3absV70,V[,96],intlogV52V96,V[,158],intV96V158,squareV163,pabsV169,cubeV188,V[,192],intcubeV188V192)
...
intlogp1V7V96<-V[,96]/(logp1V7+0.5)

squareV188<-V[,188]^2

VYa<-cbind(logp1V7,V[,10],ooverV16,sqrtV25,logV52,V[,96],intlogp1V7V96,V[,158],squareV163,squareV188,V[,192],intcubeV188V192)

VY<-data.frame(1,VYa)

meany1<-apply(t(t(VY)*ycoef),1,sum)-0.75*A-0.25*A*V[,192]+0.60*A*V[,51]

err1<-rnorm(n,sd=0.05)
err0<-rnorm(n,sd=0.1)
err<-(1-A)*err0+A*err1
Y<-meany1+err

meany11<-apply(t(t(VY)*ycoef),1,sum)-0.75*1-0.25*1*V[,192]+0.60*1*V[,51]
meany10<-apply(t(t(VY)*ycoef),1,sum)-0.75*0-0.25*0*V[,192]+0.60*0*V[,51]
...
csvFileName <- paste("CHDScenario15DS",k,".csv",sep="")
...
DS_scenario15<-DS
save(DS_scenario15,file="DS_scenario15.Rdata")
ate_scenario15<-ate
save(ate_scenario15,file="ate_scenario15.Rdata")

#> summary(ate)
#Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#-0.22934 -0.17473 -0.16031 -0.16058 -0.14307 -0.09183 