...
load("pcoef_scenario12.Rdata")
load("ycoef_scenario12.Rdata")

pcoef<-pcoef_scenario12
ycoef<-ycoef_scenario12
...
Vtreatment<-cbind(1,intcontV5,logp1V7,ooverV16,sqrtV25,pabsV51,V[,96],intlogp1V7V96,V[,156],squareV163)
...
VYa<-cbind(intcontV5,sqrtV25,V[,96],squareV163,pabsV169,cubeV188)

VY<-data.frame(1,VYa)

meany1a<- apply(t(t(VY)*ycoef),1,sum)-0.5*A
meany1<-3/(exp(-0.6*meany1a))

err<-rnorm(n,sd=2) 

Y<-meany1+err

meany11a<-apply(t(t(VY)*ycoef),1,sum)-0.5*1
meany11<-3/(exp(-0.6*meany11a))

meany10a<-apply(t(t(VY)*ycoef),1,sum)-0.5*0
meany10<-3/(exp(-0.6*meany10a))
...
csvFileName <- paste("CHDScenario12DS",k,".csv",sep="")
...
DS_scenario12<-DS
save(DS_scenario12,file="DS_scenario12.Rdata")
ate_scenario12<-ate
save(ate_scenario12,file="ate_scenario12.Rdata")

#> summary(ate)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-3.807  -3.170  -3.132  -3.159  -3.113  -3.059 
