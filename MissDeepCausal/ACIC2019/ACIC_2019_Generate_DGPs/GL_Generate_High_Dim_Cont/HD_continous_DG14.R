...
load("pcoef_scenario14.Rdata")
load("ycoef_scenario14.Rdata")

pcoef<-pcoef_scenario14
ycoef<-ycoef_scenario14
...
logp1V8<-log(V[,8])
ooversqrtV17<-1/sqrt(V[,17])
expV52<-exp(V[,52])
absV157<-abs(V[,157])
intlogp1V8V157<-V[,157]*logp1V8
intV100V165<-V[,100]*V[,165]

Vtreatment<-cbind(1,logp1V8,ooversqrtV17,expV52,V[,100],absV157,intlogp1V8V157,V[,162],V[,165],intV100V165)
...
sqrtV17<-sqrt(V[,17])
sdV52<-4*sd(V[,52])
indsquareV188<-(V[,188]^2)*(V[,188]>0.4)

VYa<-cbind(logp1V8,sqrtV17,V[,52],absV157,V[,162],V[,166],indsquareV188)

VY<-data.frame(1,VYa)

meany1a<-apply(t(t(VY[1:5])*ycoef[1:5]),1,sum)+0.75*A
meany1b<-abs(apply(t(t(VY[6:8])*ycoef[6:8]),1,sum))+10*A
meany1<-exp(meany1a)+meany1b

err<-rnorm(n,sd=4) 
Y<-meany1+err

meany11a<-apply(t(t(VY[1:5])*ycoef[1:5]),1,sum)+0.75*1
meany11b<-abs(apply(t(t(VY[6:8])*ycoef[6:8]),1,sum))+10*1
meany11<-exp(meany11a)+meany11b

meany10a<-apply(t(t(VY[1:5])*ycoef[1:5]),1,sum)+0.75*0
meany10b<-abs(apply(t(t(VY[6:8])*ycoef[6:8]),1,sum))+10*0
meany10<-exp(meany10a)+meany10b
...
csvFileName <- paste("CHDScenario14DS",k,".csv",sep="")
...
DS_scenario14<-DS
save(DS_scenario14,file="DS_scenario14.Rdata")
ate_scenario14<-ate
save(ate_scenario14,file="ate_scenario14.Rdata")

#> summary(ate)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#57.45   60.38   61.13   61.11   61.78   64.43 
