...
load("pcoef_scenario10.Rdata")
load("ycoef_scenario10.Rdata")

pcoef<-pcoef_scenario10
ycoef<-ycoef_scenario10
...
intabsV157V165<-absV157*V[,165]

Vtreatment<-cbind(1,V[,1],logp1V8,ooversqrtV17,expnV52,absV157,intlogp1V8V157,V[,164],V[,165],intabsV157V165)
...
intV157V166<-V[,157]*V[,166]
VYa<-cbind(V[,8],V[,17],V[,52],V[157],V[,166],intV157V166)

VY<-data.frame(1,VYa)

meany1<-apply(t(t(VY)*ycoef),1,sum)+5*A+V[,8]*5*A+intV157V166*0.5*A

err<-rt(n,df=4)  

Y<-meany1+err

meany11<-apply(t(t(VY)*ycoef),1,sum)+5*1+V[,8]*5*1+intV157V166*0.5*1
meany10<-apply(t(t(VY)*ycoef),1,sum)+5*0+V[,8]*5*0+intV157V166*0.5*0
...
csvFileName <- paste("CHDScenario10DS",k,".csv",sep="")
...
DS_scenario10<-DS
save(DS_scenario10,file="DS_scenario10.Rdata")
ate_scenario10<-ate
save(ate_scenario10,file="ate_scenario10.Rdata")

#> summary(ate)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#8.882   9.079   9.134   9.134   9.204   9.393 
