...
load("pcoef_scenario9.Rdata")
load("ycoef_scenario9.Rdata")

pcoef<-pcoef_scenario9
ycoef<-ycoef_scenario9
...
intV164V165<-V[,164]*V[,165]

Vtreatment<-cbind(1,logp1V8,ooversqrtV17,expnV52,absV157,intlogp1V8V157,V[,164],V[,165],intV164V165)
...
VYa<-cbind(V[,8],V[,17],V[,52],V[157],V[,166])
VY<-data.frame(1,VYa)

meany1a<-apply(t(t(VY[1:3])*ycoef[1:3]),1,sum)+0.5*A
meany1b<-apply(t(t(VY[4:6])*ycoef[4:6]),1,sum)
meany1<-exp(meany1a)+(meany1b+59)*(meany1b>-59.5)

err<-rnorm(n,sd=3) 

Y<-meany1+err

meany11a<-apply(t(t(VY[1:3])*ycoef[1:3]),1,sum)+0.5*1
meany11b<-apply(t(t(VY[4:6])*ycoef[4:6]),1,sum)
meany11<-exp(meany11a)+(meany11b+59)*(meany11b>-59.5)

meany10a<-apply(t(t(VY[1:3])*ycoef[1:3]),1,sum)+0.5*0
meany10b<-apply(t(t(VY[4:6])*ycoef[4:6]),1,sum)  
meany10<-exp(meany10a)+(meany10b+59)*(meany10b>-59.5)
...
csvFileName <- paste("CHDScenario9DS",k,".csv",sep="")
...
DS_scenario9<-DS
save(DS_scenario9,file="DS_scenario9.Rdata")
ate_scenario9<-ate
save(ate_scenario9,file="ate_scenario9.Rdata")

#> summary(ate)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#12.41   12.57   12.61   12.62   12.67   12.93 
