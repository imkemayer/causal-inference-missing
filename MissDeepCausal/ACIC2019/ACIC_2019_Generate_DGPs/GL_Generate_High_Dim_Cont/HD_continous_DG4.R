...
load("pcoef_scenario4.Rdata")
load("ycoef_scenario4.Rdata")

pcoef<-pcoef_scenario4
ycoef<-ycoef_scenario4
...
Vtreatment<-cbind(1,V[,2],V[,5],intV2V5,V[,12],V[,23],V23square,intV12V23square,sqrtV67,V[,77],IndV89,
...
VYa<-cbind(V[,2],V[,5],intV2V5,V[,12],V[,23],V23square,intV12V23square,V[,40],sqrtV67,V[,77],IndV89,
           IndcontV95,expV106,V[,122],V[,133],V[,146],intV122V146,V[,150],V[,168],V[,198],IndV199)

VY<-data.frame(1,VYa)

meany1<- apply(t(t(VY)*ycoef),1,sum)+2.1*A

err<-rt(n,df=5) 

Y<-meany1+err

meany11<- apply(t(t(VY)*ycoef),1,sum)+2.1*1
meany10<- apply(t(t(VY)*ycoef),1,sum)+2.1*0
...
# Create CSV files
csvFileName <- paste("CHDScenario4DS",k,".csv",sep="")
...
DS_scenario4<-DS
save(DS_scenario4,file="DS_scenario4.Rdata")

ate_scenario4<-ate
save(ate_scenario4,file="ate_scenario4.Rdata")
