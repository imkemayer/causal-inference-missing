...
option_list = list(
  ...
  make_option(c("-w"), type="logical", default=TRUE, 
...
# for (id in 0:4){
#   opt <- data.frame(cbind("link" = "linear0", "setting"="dlvm2", 
#                           "nsample" = 10000, "ncov"= 10, "nlat" = 3,
#                           "mech" = "MCAR", "percmiss" = 0.3, "mask"=FALSE, "w"=FALSE, 
#                           "dmiwae"=10, "hmiwae"=128, "nzmult"=200,"cit"=FALSE, "cit2"=FALSE,
#                           "ci2_imp"="mice","offset"=TRUE,"out_date"="2019-09-24", "id"=id))
#   opt[,c(1,2,6,15,17)] <- sapply(opt[,c(1,2,6,15,17)], as.character)
#   opt[,c(3,4,5,7,10,11,12,18)] <- sapply(opt[,c(3,4,5,7,10,11,12,18)], function(x) as.numeric(as.character(x)))
#   opt[,c(8,9,13,14,16)] <- sapply(opt[,c(8,9,13,14,16)],function(x) as.logical(as.character(x)))
...
data_zhat_miwae <- read.csv(file = paste0(data.dir, setting, "_", n, "_", p, "_", d, 
                                          use.w, "_dmiwae", d.miwae, "_hmiwae", h.miwae, add_mask, 
                                          "_seed", id, 
                                          "_propNA", format(round(perc.miss, 2), nsmall=2),
                                          "_zhat.csv"), 
                            sep = ";", header=F)
...
save(tau.lin, file = paste0(rdata.dir, opt$out_date, 
                            "_",link,"_",setting, 
                            "_n", n, "_p", p, "_d", d, 
                            use.w, "_dmiwae", d.miwae, "_hmiwae", h.miwae,
                            add_mask,
