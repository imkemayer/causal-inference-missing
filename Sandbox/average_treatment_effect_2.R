...
#' @param ignore_clusters Specifies whether to ignor the clusters in the causal forest,
#'               for instance, when subset corresponds exactly to 1 cluster
#'
#' @examples
#' \dontrun{
...
#' average_treatment_effect_2(c.forest, target.sample = "all")
#'
...
#' average_treatment_effect_2(c.forest, target.sample = "treated")
#'
#' # Estimate the conditional average treatment effect on samples with positive X[,1].
#' average_treatment_effect_2(c.forest, target.sample = "all", X[, 1] > 0)
#' }
#'
...
#' @export
average_treatment_effect_2 <- function(forest,
  ...
  cluster.se <- (!ignore_clusters) & length(forest$clusters) > 0
