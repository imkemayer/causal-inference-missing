...
for (mech in c("MCAR", "MAR", "MNAR")){
  for (prop.missing in c(0.1, 0.3, 0.5, 0.7)){
      ...
      for (cit in c(FALSE,TRUE)){
        for (cio in c(FALSE,TRUE)){
          ...
          #writeLines(paste0("cit: ", cit,", cio: ", cio, ", use.mask: ", use.mask))
            ...
            generateDataForIterInCurrentEnvironment(seed, x[,1:6], z, w, 
                                                    mask = is.na(x.miss[,1:6]),cit=cit,cio=cio,
                                                    overlap = "all")
            x.miss <- x.miss[,1:6]
            for (imp in imp.methods){
              ...
              if (imp != "mice"){
                res <- ipw(X=data.frame(x.imp), outcome = y, treat = z.r,
                           ...
                           mask = mask,
                           fitted = fitted)
                tmp1 <- cbind(res[1], NA,"ipw1", imp, mech, prop.missing, use.mask, FALSE, paste0(cit,".", cio), cit, cio)
                colnames(tmp1) <- col.names
                
                tmp2 <- cbind(res[2], NA,"ipw2", imp, mech, prop.missing, use.mask, FALSE, paste0(cit,".", cio), cit, cio)
                ...
                try(res <- dr(X=data.frame(x.imp), outcome = y, treat = z.r,
                              ...
                              mask = mask,
                              fitted = fitted))
                tmp3 <- cbind(res$dr, res$se,"dr", imp, mech, prop.missing, use.mask, FALSE, paste0(cit,".", cio), cit, cio)
                ...
                for (use.outcome in c(F,T)){
                  ...
                  tmp1 <- cbind(mean(res[,1]), NA, "ipw1", imp, mech, prop.missing, use.mask, use.outcome, paste0(cit,".", cio), cit, cio)
                  colnames(tmp1) <- col.names
                  
                  tmp2 <- cbind(mean(res[,2]), NA, "ipw2", imp, mech, prop.missing, use.mask, use.outcome, paste0(cit,".", cio), cit, cio)
                  colnames(tmp2) <- col.names
                  ...
                  tmp3 <- cbind(mean(res[,1]), mean(res[,2]), "dr", imp, mech, prop.missing, use.mask, use.outcome, paste0(cit,".", cio), cit, cio)
                  colnames(tmp3) <- col.names
    ...
    save(results_compare_dr_miss,
         ...
         N,
         prop.missing,
         file = paste0("./RData/", Sys.Date(), "_ihdp_N", N, "_", mech, "_propNA",
