# https://github.com/vdorie/npci/blob/master/examples/ihdp_sim/data.R (2019-05-16)
loadDataInCurrentEnvironment <- function(covariates = "select", p.score = "none") {
  ...
  if (is.character(covariates) && covariates != "full") {
    ...
    x <- transform(x, trans$standardize)
    z <- ihdp$treat
...
generateDataForIterInCurrentEnvironment <- function(iter, x, z, w, mask=NULL, cit = NULL, cio = NULL, overlap = TRUE, covariates = "select", setting = "A", p.score = "none") {
  ...
  if (setting == "A") {
    ...
    environment(f.0) <- args2env(baseenv(), w = c(0, w), beta, omega)
