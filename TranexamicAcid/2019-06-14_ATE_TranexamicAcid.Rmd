...
load(paste0(data.dir, Sys.Date(),"_ncomp_all_2outcomes.RData"))
...
load(paste0(data.dir, Sys.Date(),"_ncomp_all_1outcome.RData"))
...
```{r plot_ncomp_all_mf}
load(paste0(data.dir, Sys.Date(),"_ncomp_all_confounders.RData"))
...
results_ipw_ate_list <- list()
results_ipw_ate <- data.frame(matrix(ncol=7, nrow=0), row.names = NULL)
colnames(results_ipw_ate) <- c("Estimand",
                               "Imputation.method", 
                               "Imputation.ind", 
                               "Imputation.var", 
                               "PS.estimation",
...
data_list <- list(list("FAMD", "all", "2outcomes", imputed_all_2outcomes_FAMD),
                  list("FAMD", "all", "1outcome", imputed_all_1outcome_FAMD),
                  list("mice", "all", "2outcomes", imputed_all_2outcomes_mice),
                  list("mice", "all", "1outcome", imputed_all_1outcome_mice))
...
for (i in 1:length(data_list)){
  ...
  for (j in 1:length(methods_list)){
    results_ipw_ate_list[[idx]] <- ipw(X=df, outcome = outcome, treat = treatment,
                                       ps.method = methods_list[[j]], target = "all", 
                                       seed=0, trimming_weight=.999)
    results_ipw_ate <- rbind(results_ipw_ate,
                             data.frame("Estimand" = "ATE",
                                        ...
                                        "ATE.p100" = results_ipw_ate_list[[idx]][[2]]*100,
                                        "STD.p100" = results_ipw_ate_list[[idx]][[3]]*100))
...
knitr::kable(results_ipw_ate, caption = "IPW estimations", digits = 2)
...
misaem.results.all <- prepare_data_misaem(covariates_misaem, use_targets = FALSE)

data_list <- list(list("all", misaem.results.all))
...
data_list <- list(list("mia", "target", "-", get_imputeInf(data_indTBI)))
