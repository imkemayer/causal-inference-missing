---
title: "Treatment Effect of Tranexamic Acid on TBI patients with overlap weights - Bootstraping"
author: "Imke Mayer"
date: "07/31/2019"
output: 
  html_document:
...
**Same as 2019-07-24_Bootstrap_ATO_TranexamicAcid.Rmd except for adding `Remplissage.total.cristalloides` and `Remplissage.total.colloides` as confounders.**
        ...
        if (tolower(imputation)== "mice"){
          ...
          for (k in 1:length(sample$imputed)){
            tmp <- rbind(tmp, ipw(X=df.confounders[[k]], outcome = outcome, treat = treatment,
                                  ...
                                  ps.method = method, target = "overlap", 
                                  seed=0, trimming_weight=1))
          }
...
writeLines(paste0("\t CI for ATO when estimating PS with logistic regression: [", 
...
writeLines(paste0("\t CI for ATO when estimating PS with grf: [", 
...
save(bs.ipw, bs.dr, file = paste0("~/Documents/TraumaMatrix/CausalInference/Simulations/causal-inference-missing/TranexamicAcid/RData/",Sys.Date(),"_bootstrap4_ato_",population,"patients.RData"))
```
...
plot.ipw.glm <- ggplot(data=data.frame(bs.ipw.glm)) + 
  ...
  labs(title="IPW ATO with PS via logistic regression", cex=0.8)
}
plot.ipw.glm
...
plot.ipw.grf <- ggplot(data=data.frame(bs.ipw.grf)) + 
  ...
  theme(axis.text=element_text(size=15)) +
  facet_grid(~as.factor(tolower.imputation.)) +
  labs(title="IPW ATO with PS via GRF", cex=0.8)
}
...
plot.dr.glm <- ggplot(data=data.frame(bs.dr.glm)) + 
  ...
  labs(title="DR ATO with PS via GRF", cex=0.8)
}
plot.dr.glm
