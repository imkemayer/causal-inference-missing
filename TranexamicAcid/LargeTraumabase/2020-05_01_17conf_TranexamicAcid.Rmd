---
title: "17 confounders - Treatment Effect of Tranexamic Acid on TBI patients - on large Traumabase (20,000 patients)"
author: "Imke Mayer"
date: "05/01/2020"
output: 
  html_document:
...
**We use 17 confounders.**

**We use the following set of confounders in this analysis: `Trauma.center`, `Cardiac.arrest.ph`, `SBP.ph.min, DBP.ph.min, HR.ph.max`, `SBP.ph, DBP.ph, HR.ph`, `Shock.index.ph` (`HR.ph`/`SBP.ph`), `Cristalloid.volume`, `Colloid.volume`, `SpO2.ph.min`, `Vasopressor.therapy`, `HemoCue.init`, `Delta.hemoCue`, `AIS.external`, `Activation.HS.procedure`.**

**Add `Activation.HS.procedure`**
...
Look at the `Activation.HS.procedure` variable: how is the treatment distributed w.r.t. this variable?
...
```{r}
sprintf("Mortality rate: %2.1f %%", mean(imputed_FAMD$Death==1)*100)
...
### FAMD
```{r difference_in_means_adj_famd-grf, echo=F}
...
### MICE
```{r difference_in_means_adj_mice-grf, echo=F}
w.hat <- list()
weights <- list()
df.ps <- list()
...
### GRF - MIA
```{r difference_in_means_adj_grf-mia, echo=FALSE}
  ...
  for (i in 1:length(data_list)){
    if (data_list[[i]][[1]]=="mice"){
      ...
      for (k in 1:length(data_list[[i]][[3]])){
        ...
        treatment[[k]] <- as.numeric(data_list[[i]][[3]][[k]]$Tranexamic.acid)
        outcome[[k]] <- as.numeric(data_list[[i]][[3]][[k]]$Death)
      }
      ...
      treatment <- as.numeric(data_list[[i]][[3]]$Tranexamic.acid)
      outcome <- as.numeric(data_list[[i]][[3]]$Death)
    }
    ...
    if (data_list[[i]][[1]]== "mice"){
      ...
      for (k in 1:length(data_list[[i]][[3]])){
        ...
        tmp <- rbind(tmp, ipw(X=df[[k]], outcome = outcome[[k]], treat = treatment[[k]],
                                         mask = mask[[k]],
                                         ps.method = methods_list[[j]], target = "control", 
...
weights_list <- list("control")#list("all", "treated", "control", "overlap", "matching")
weights_trim_list <- list(0.999, 0.999, 0.999, 1, 1)
estimands_list <- list("ATC")#list("ATE", "ATT", "ATC", "ATE_overlap","ATE_match")
...
for (use.mask in c(TRUE)){
  for (k in 1:length(weights_list)){
    ...
    if (population == "tbi"){
      treatment <- data_indTBI$Tranexamic.acid
      outcome <- data_indTBI$Death
      df <- data_indTBI %>%
              dplyr::select(-c("Tranexamic.acid", "Death")) %>%
              dplyr::select(confounders)
    } else {
      treatment <- data_indAll$Tranexamic.acid
      outcome <- data_indAll$Death
      df <- data_indAll %>%
              dplyr::select(-c("Tranexamic.acid", "Death")) %>%
              dplyr::select(confounders)
    }
...
results_ipw_mean <- data.frame(matrix(ncol=8, nrow=0), row.names = NULL)
...
# weights_trim_list <- list(0.999, 0.999, 0.999, 1, 1)
# estimands_list <- list("ATE")#list("ATE", "ATT", "ATC", "ATE_overlap","ATE_match")
# 
#     
# idx <- 1
# 
...
#   for (k in 1:length(weights_list)){
#     for (i in 1:length(data_list)) {
#     treatment <- data_list[[i]][[3]]$Tranexamic.acid
#     outcome <- data_list[[i]][[3]]$Death
# 
# 
#     df <- data_list[[i]][[3]] %>%
#               dplyr::select(-c("Tranexamic.acid", "Death")) %>%
